...
namespace DBD_API.Modules.Steam.ContentManagement
{
    ...
    class ProtoManifest
    {
        ...
        public static ProtoManifest LoadFromBuffer(byte[] buffer, out byte[] checksum)
        {
            using (var uncompressed = new MemoryStream())
            {
                ...
                checksum = ShaHash(uncompressed.ToArray());
                uncompressed.Seek(0, SeekOrigin.Begin);
                return ProtoBuf.Serializer.Deserialize<ProtoManifest>(uncompressed);
            }
        }
        ...
        public void SaveToBuffer(out byte[] buffer, out byte[] checksum)
        {
